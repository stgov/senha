...
class PersonReID:
    ...
    def extract_embedding(
        ...
        hog_features = hog_features[:64] if len(hog_features) >= 64 else np.pad(hog_features, (0, 64 - len(hog_features)))
            ...
            if results['ids'] and len(results['ids'][0]) > 0:
                ...
                for i, (matched_id, distance) in enumerate(zip(results['ids'][0], results['distances'][0])):
                    ...
                    if DEBUG_MODE and i == 0:
                        print(f"🔍 Match: {matched_id} | Similitud: {similarity:.3f} | Umbral: {self.similarity_threshold}")
                    
                    # Si la persona está marcada, siempre mantener su ID
                    if matched_id in self.marked_ids and similarity >= (self.similarity_threshold - 0.1):
                        if DEBUG_MODE:
    ...
    def _init_kalman_embedding(
        ...
        kalman.processNoiseCov = np.eye(embedding_dim, dtype=np.float32) * KALMAN_EMBEDDING_PROCESS_NOISE
        
        # Covarianza del ruido de medición (confianza en la observación actual)
        kalman.measurementNoiseCov = np.eye(embedding_dim, dtype=np.float32) * KALMAN_EMBEDDING_MEASUREMENT_NOISE
...
def main():
    ...
    print(f"  • BBoxSmoother para bbox (dead_zone={BBOX_DEAD_ZONE}px, factor={BBOX_SMOOTHING_FACTOR})")
    if USE_KALMAN_EMBEDDINGS:
        print(f"  • Kalman Filter para embeddings temporales (proceso={KALMAN_EMBEDDING_PROCESS_NOISE}, medición={KALMAN_EMBEDDING_MEASUREMENT_NOISE})")
    ...
    bbox_smoother = BBoxSmoother(
        dead_zone=BBOX_DEAD_ZONE,
        smoothing_factor=BBOX_SMOOTHING_FACTOR
    )
    ...
    box_annotator_red = sv.BoxAnnotator(thickness=4, color=sv.Color.RED)  # ROJO para personas marcadas
        ...
        while cap.isOpened():
            ...
            if show_all_detections and original_count > 0:
                ...
                if results is not None and len(results) > 0:
                    for result in results:
                        ...
                        color = (0, 255, 0) if class_id == 0 else (0, 0, 255)  # Verde para personas, rojo para otros
                ...
                if OPTIMIZE_REID and tracker_id is not None:
                    ...
                    if tracker_id in tracker_to_reid:
                        ...
                        if person_id in person_reid.persons:
                            ...
                            if person_reid.bbox_smoother is not None and record.smoother_idx is not None:
                                record.predicted_bbox = person_reid.bbox_smoother.smooth(record.smoother_idx, bbox)
                            else:
                                record.predicted_bbox = bbox
                        
                        if DEBUG_MODE and frame_index % 90 == 0:
                            print(f"⚡ Re-usando ID: tracker_{tracker_id} → {person_id} (sin Re-ID)")
                        continue
                    ...
                    if gesture_result.gestures and gesture_result.hand_landmarks:
                        for gesture_list, hand_landmarks in zip(
                            ...
                            if (top_gesture.category_name == "Closed_Fist" and
                                top_gesture.score >= GESTURE_CONFIDENCE):
            ...
            if len(detections_normal) > 0:
                ...
                annotated_frame = box_annotator_green.annotate(
                    ...
                    detections=detections_normal_obj
                )
            ...
            if len(detections_marked) > 0:
                ...
                annotated_frame = box_annotator_red.annotate(
                    scene=annotated_frame,
                    detections=detections_marked_obj
                )
                annotated_frame = label_annotator.annotate(
                    scene=annotated_frame,
                    detections=detections_marked_obj,
                    labels=labels_marked
                )
            
            # ====== DIBUJAR KEYPOINTS DE MANOS Y GESTOS ======
            for person_id, (hand_landmarks, gesture_name, confidence, offset) in gesture_annotations.items():
                ...
                h, w = frame.shape[:2]
                bbox = person_reid.persons[person_id].last_bbox if person_id in person_reid.persons else None
                if bbox:
