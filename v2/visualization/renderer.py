...
class Visualizer:
    ...
    def draw_gesture_label(
        ...
        cv2.rectangle(
            frame, (text_x, text_y - text_h - 5), (text_x + text_w + 5, text_y + 5), (0, 0, 0), -1
        )

        cv2.putText(
            frame, text, (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2
        )
    ...
    def draw_overlay(
        ...
        cv2.putText(
            frame,
            (
                f"Re-ID: {stats['total']} total | "
                f"{stats['active']} activas | "
                f"{stats['marked']} marcadas"
            ),
            (10, y),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.6,
        ...
        reid_mode = (
            "Optimizado (entrada/salida)" if self.config.optimize_reid else "Completo (cada frame)"
        )
        cv2.putText(
            frame,
