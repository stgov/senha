...
# para que los imports funcionen al ejecutar este script directamente.
...
class ReIDSystem:
    ...
    def __init__(
        ...
        self.bbox_smoother = BBoxSmoother(
            dead_zone=self.config.bbox_dead_zone, 
            smoothing_factor=self.config.bbox_smoothing_factor
        )
    ...
    def setup_video(self) -> bool:
        ...
        print(f"🧠 Re-ID: umbral={self.config.similarity_threshold:.2f}, "
              f"optimizado={self.config.optimize_reid}")
        print(f"👋 Gestos: {self.config.gesture_model_path}")
        print(f"📦 Kalman: process_noise={self.config.kalman_embedding_process_noise}, "
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default='0',
        help='Fuente de video (0=webcam, path=archivo)'
    )
    parser.add_argument(
        '--model',
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.70,
        help='Umbral de similitud Re-ID'
    )
    parser.add_argument(
        '--no-optimize',
        action='store_true',
        help='Desactivar optimización Re-ID'
    )
    parser.add_argument(
        '--conf',
        type=float,
        default=0.3,
        help='Confidence threshold para detección'
    )
    parser.add_argument(
        '--iou',
        type=float,
